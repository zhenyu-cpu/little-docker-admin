<template>
	<div class="home-container layout-pd">
		<el-row :gutter="15" class="home-card-one mb15">
			...
			>
              ...
              <img v-else style="borderRadius: 50%" src="https://img2.baidu.com/it/u=1978192862,2048448374&fm=253&fmt=auto&app=138&f=JPEG?w=504&h=500" class="layout-navbars-breadcrumb-user-link-photo mr5 font32" />
						</div>
					</div>
				</div>
			</el-col>
		...
		<el-row :gutter="15" class="home-card-two mb15">
          ...
          <div class="home-card-item-title">宿主机详情</div>
          <div class="home-monitor">
						<div class="flex-warp">
							<div class="flex-warp-item" v-for="(v, k) in state.homeFour" :key="k">
								<div class="flex-warp-item-box" :class="`home-animation${k}`">
									<div class="flex-margin">
										<span class="pl5">{{ v.title }}</span>
    ...
    <el-row :gutter="15" class="home-card-three">
      <el-col :xs="24" :sm="10" :md="10" :lg="8" :xl="8">
        <div class="home-card-item">
					<div class="home-card-item-title">集群节点</div>
					<div class="system-list">
            <div v-for="(v, k) in state.homeFive" :key="k">
              <div class="only-one" :style="{borderTop: k == 0 ? '1px rgb(223, 223, 223) solid' : ''}">
                <span>
                  <el-button :type="v.isWork ? 'primary':'danger'" plain size="small">{{ v.isWork ? 'worker' : 'leader' }}</el-button>
                </span>
                <span>{{ v.path }}</span>
              </div>
            </div>
...
const state = reactive({
  ...
  homeFive: [
    {
      isWork: true,
      path: "192.168.100.104"
    },
    {
      isWork: true,
      path: "192.168.100.238"
    },
    {
      isWork: false,
      path: "192.168.100.42"
...
const initPieChart = () => {
	...
	var getname = ['运行的容器', '暂停的容器', '停止的容器'];
	var getvalue = [50, 0, 7];
	var data = [];
	for (var i = 0; i < getname.length; i++) {
		data.push({ name: getname[i], value: getvalue[i] });
	}
	const colorList = ['#36C78B', '#51A3FC', '#FEC279'];
	const option = {
		backgroundColor: state.charts.bgColor,
		title: {
			text: '容器数据',
